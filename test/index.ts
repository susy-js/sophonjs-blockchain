...
import { srlp, toBuffer } from 'sophonjs-util'
  ...
  async.series(
    [
      ...
      function addBlockWithBody(done) {
        ...
        const genesisBlock = new Block(Buffer.from(testData.genesisSRLP.slice(2), 'hex'))
        blockchain.putGenesis(genesisBlock, (err?: Error) => {
          if (err) {
            return done(err)
          }
          const block = new Block(Buffer.from(testData.blocks[0].srlp.slice(2), 'hex'))
      ...
      function immutableCachedObjects(done) {
        const blockchain = new Blockchain({ validate: false })
        // clone blocks[1]
        const testBlock = new Block(srlp.decode(srlp.encode(blocks[1].raw)))
  ...
  db.batch(
    [
      ...
      },
      {
        ...
        keyEncoding: 'binary',
        valueEncoding: 'binary',
        value: srlp.encode(genesis.header.raw),
      },
      {
        type: 'put',
        key: Buffer.from(
        ...
        keyEncoding: 'binary',
        valueEncoding: 'binary',
        value: srlp.encode(new BN(17179869184).toBuffer()),
      },
      {
        type: 'put',
        key: Buffer.from(
        ...
        keyEncoding: 'binary',
        valueEncoding: 'binary',
        value: srlp.encode(genesis.serialize(false).slice(1)),
      },
      {
        type: 'put',
        key: 'heads',
