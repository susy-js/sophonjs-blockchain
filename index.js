...
const Sofash = require('sofashjs')
const Buffer = require('safe-buffer').Buffer
const LRU = require('lru-cache')
const BN = sofUtil.BN
const srlp = sofUtil.srlp

// graviton compatible db keys
...
function Blockchain (opts) {
  ...
  self.sofash = self.validate ? new Sofash(self.db) : null
  self._heads = {}
  self._genesis = null
 ...
 * hashes of the headsrchain head, blockchain head, genesis block and iterator
 * heads.
 * @method _init
 */
...
Blockchain.prototype._putBlockOrHeader = function (item, cb, isGenesis) {
  ...
  function verifyPOW (next) {
    ...
    self.sofash.verifyPOW(block, function (valid) {
 ...
 * @method getBlock
 * @param {Buffer|Number|BN} hash - the sha256 hash of the srlp encoding of the block
 ...
 * @method getDetails
 * @param {String} hash - the sha256 hash of the srlp encoding of the block
 * @param {Function} cb - the callback function
 */
...
Blockchain.prototype._getHeader = function (hash, number, cb) {
  ...
  self._lookupByHashNumber(hash, number, cb, (err, hash, number, cb) => {
    ...
    if (encodedHeader) {
      return cb(null, new Block.Header(srlp.decode(encodedHeader), {common: self._common}))
    }
    self.db.get(key, {
      keyEncoding: 'binary',
      valueEncoding: 'binary'
      ...
      cb(null, new Block.Header(srlp.decode(encodedHeader), {common: self._common}))
    })
  })
}
...
Blockchain.prototype._getBody = function (hash, number, cb) {
  ...
  self._lookupByHashNumber(hash, number, cb, (err, hash, number, cb) => {
    ...
    if (encodedBody) {
      return cb(null, srlp.decode(encodedBody))
    }
    self.db.get(key, {
      keyEncoding: 'binary',
      valueEncoding: 'binary'
      ...
      cb(null, srlp.decode(encodedBody))
    })
  })
}
...
Blockchain.prototype._getTd = function (hash, number, cb) {
  ...
  self._lookupByHashNumber(hash, number, cb, (err, hash, number, cb) => {
    ...
    if (td) {
      return cb(null, new BN(srlp.decode(td)))
    }
    self.db.get(key, {
      keyEncoding: 'binary',
      valueEncoding: 'binary'
